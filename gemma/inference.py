...
class InferenceInput:
    def __init__(self, _id, input_text):
        self._id = _id
...
def create_example(all_data, instruction, tokenizer):
    ...
    for data in tqdm(all_data):
        ...
        response = "<Answer>: {}\n<Reason>: {}".format(answer, supporting_sentence)
        ...
        all_result.append(InferenceInput(_id = data_id, input_text= tokenizer.apply_chat_template(messages, tokenize=False)))
...
def generate_batch_answer(batches, tokenizer, model):
    for batch_num, batch in enumerate(tqdm(batches)):
        ...
        decoded_outputs = [tokenizer.decode(output[len(inputs[i]):], skip_special_tokens=True) for i, output in enumerate(outputs)]
...
Read the provided **Related Document**, find the **Answer** to the given **Question**, and extract it from the **Related Document**. Additionally, extract the sentence number that is crucial for inferring the <Answer> from the <Related Document> and write it in the <Supporting Sentence>. You must strictly follow the <Output format> and provide valid information for all items. There is nothing that cannot be filled.
...
"""
    ...
    print(batch_size)
    tokenizer = AutoTokenizer.from_pretrained(model_path)
    model = AutoModelForCausalLM.from_pretrained(model_path)  #gemma는 encode-only 구조
    ...
    #### 답변작성
    output_path = "gemma_answer_zero_ex.json"
...
# [1] Arthur's Magazine (1844–1846) was an American literary periodical published in Philadelphia in the 19th century.
# [2]  Edited by T.S. Arthur, it featured work by Edgar A. Poe, J.H. Ingraham, Sarah Josepha Hale, Thomas G. Spear, and others.
# [3]  In May 1846 it was merged into "Godey's Lady's Book".

# Document 8 : First for Women
# [4] First for Women is a woman's magazine published by Bauer Media Group in the USA.
# [5]  The magazine was started in 1989.
# [6]  It is based in Englewood Cliffs, New Jersey.

# <Output format>
# <Answer>: Arthur's Magazine
# <Reason>: [1] Arthur's Magazine (1844–1846) was an American literary periodical published in Philadelphia in the 19th century.
# [4] First for Women is a woman's magazine published by Bauer Media Group in the USA.